...
policy (run_gloves_policy, scored via density non-conformity).
...
from density_calc_for_multitaskditpolicy import compute_density_score, generate_action_chunk

# NOTE. Idk how to fix this lerobot issue?
...
#             raise RuntimeError(
#                 "run_gloves_policy(..., not_blend=True) requires obs_history (an n_obs_steps-long "
...
if __name__ == "__main__":
    ...
    for trial in range(args_cli.num_rollouts):
        ...
        if policy_backbone == "fm":
            #policy = DiTPolicy.from_pretrained(args_cli.fm_checkpoint) # NOTE: this is DiTPolicy = GLOVES

            policy = MultiTaskDiTPolicy.from_pretrained(
                args_cli.fm_checkpoint
            )

            print("multitaskditpolicy, ", policy)
            policy.to(device)
            policy.eval()
            ...
            # Build your LeRobot-format observation here
