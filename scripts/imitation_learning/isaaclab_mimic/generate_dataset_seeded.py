...
parser.add_argument(
    ...
    default=None,
    help=(
        "Override datagen_config.seed for this run. Leave unset to use the value baked into the"
        " environment config. Two runs sharing a seed produce identical demos, and a longer run is"
        " a prefix-extension of a shorter one, so a distinct seed is required for an independent"
        " batch of demos."
    ),
)
parser.add_argument(
...
def main():
    ...
    # Set seed for generation. --seed overrides the value from the environment config, so an
    # independent batch of demos can be generated without editing the shared cfg file.
    if args_cli.seed is not None:
        env.cfg.datagen_config.seed = args_cli.seed
    print(f"datagen seed: {env.cfg.datagen_config.seed}")
